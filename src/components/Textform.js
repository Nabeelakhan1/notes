...
export default function Textform(props) {
  ...
  <div className="container">
   <h3  style={{color:props.mode==='dark'?'white':'black'}}>Enter text here</h3>
    ...
    <button disabled={text.length===0} className="btn btn-primary m-1" onClick={handleup}>Upper-Text</button>
  <button disabled={text.length===0} className="btn btn-primary m-3" onClick={clear}>Clear</button>
  <button disabled={text.length===0} className="btn btn-primary m-3" onClick={lower}>Lower Case</button>

  <h5 style={{color:props.mode==='dark'?'white':'black'}}>Character:{text.length}</h5>
  <h5 style={{color:props.mode==='dark'?'white':'black'}}>Words:{text.split(" ").filter((element)=>{return element.length!==0}) .length}</h5>
